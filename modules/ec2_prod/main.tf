...
resource "aws_instance" "khoamd_ec2_prod" {
  ...
  tags = {
    "Owner" = local.owner
    "Name"  = "${local.owner}-prod-${local.suffixed}-tags"
...
output "prod_public_ip" {
  description = "prod public IP"
  value       = try(aws_instance.khoamd_ec2_prod.public_ip, "")
}

output "prod_host_os" {
  description = "prod host os"
  value       = try(local.host_os, "")
}

output "prod_private_key_directory" {
  description = "prod private key directory"
  value       = try(local.private_key_directory, "")
}

output "prod_owner" {
  description = "prod owner"
  value       = try(local.owner, "")
}

output "prod_suffixed" {
  description = "prod suffixed"
  value       = try(local.suffixed, "")
}

output "prod_key_pair_pem_file" {
  description = "prod key pair pem file"
